...
def pesquisar_medicamento(banco_de_dados, termo_busca):
    ...
    for med in banco_de_dados:
        ...
        if any(
            termo_busca in str(valor).lower() for valor in dados_dicionario.values()
        ):
...
def iniciar_programa():
    med1 = Medicamento(
        1, "Dipirona", 500.0, "08:00", "Todos os dias", 10, "1 comprimido"
    )
    med2 = Medicamento(
        2, "Amoxicilina", 875.0, "08:00 e 20:00", "Segunda a Sexta", 7, "1 cápsula"
    )
    med3 = Medicamento(
        3, "Ibuprofeno", 400.0, "12:00", "Dias alternados", 15, "1 comprimido"
    )
    med4 = Medicamento(
        4,
        "Vitamina C",
